...
- **Coder git SSH wrapper** - Sets `GIT_SSH_COMMAND` to use Coder's custom SSH wrapper (`/tmp/coder.*/coder gitssh --`)
- **File permissions** - Sets correct permissions on SSH keys (600 for private, 644 for public)

**Important:** The Coder git SSH wrapper (`coder gitssh`) is automatically configured via the `GIT_SSH_COMMAND` environment variable. This wrapper handles SSH authentication through Coder's infrastructure, allowing git operations to work seamlessly without manual SSH configuration.
...
**Important:** The `GIT_SSH_COMMAND` variable points to Coder's custom git SSH wrapper. This wrapper (`coder gitssh`) handles SSH authentication through Coder's infrastructure, allowing git operations to work without manual SSH agent configuration. The path `/tmp/coder.*/coder` is dynamically generated by Coder and will vary between workspaces.
...
### Problem: Git operations fail with "ssh: command not found"

**Cause:** `GIT_SSH_COMMAND` environment variable not set or pointing to invalid path

**Solution:**
```bash
# Check if GIT_SSH_COMMAND is set
echo $GIT_SSH_COMMAND

# Expected output: /tmp/coder.*/coder gitssh --
# If empty or incorrect, the Coder workspace may not be properly configured

# Verify the Coder git SSH wrapper exists
ls -la $(echo $GIT_SSH_COMMAND | awk '{print $1}')

# Test the wrapper directly
$GIT_SSH_COMMAND -T git@github.com

# If wrapper is missing, contact Coder administrator
```

**Why this matters:** Coder workspaces use a custom git SSH wrapper (`coder gitssh`) that handles SSH authentication through Coder's infrastructure. This wrapper is automatically configured via the `GIT_SSH_COMMAND` environment variable and is essential for all git operations (clone, push, pull, fetch).

---

### Problem: Git push works in home workspace but fails in task workspace

**Cause:** Task workspace may not have inherited the `GIT_SSH_COMMAND` environment variable
...
# If empty, manually set it (temporary fix)
export GIT_SSH_COMMAND="/tmp/coder.*/coder gitssh --"

# Better solution: Update Coder template to ensure environment variables
# are inherited by task workspaces. Contact Coder administrator.
```
